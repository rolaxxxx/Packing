...
PointsArrayType Math::getSpheresTouchingThreeOtherSpheres(PointType p1,
                                                          PointType p2,
                                                          PointType p3,
                                                          REAL r) {
  ...
  // cout << temporS3.x << " " << temporS3.y << " " << temporS3.z << " " <<
  // vector_len(temporS3) << endl; cout << vector_w << endl;
  vector_w = S1 * (-2);
  // cout << vector_w << endl;

  REAL calc_a, calc_b, calc_c;
  calc_a = (pow(ds2, 2) - pow(ds1, 2) + pow(S1.x, 2) - pow(S2.x, 2) +
            pow(S1.y, 2) - pow(S2.y, 2) + pow(S1.z, 2) - pow(S2.z, 2)) /
           (2 * vector_len(temporS1));
  calc_b = (pow(ds3, 2) - pow(ds1, 2) + pow(S1.x, 2) - pow(S3.x, 2) +
            pow(S1.y, 2) - pow(S3.y, 2) + pow(S1.z, 2) - pow(S3.z, 2)) /
  ...
  REAL rez_calc_a, rez_calc_b, rez_calc_c, rez_calc_d_positive,
      rez_calc_d_negative;
  rez_calc_a = (calc_a - calc_b * dot_prod(vector_u, vector_v)) /
               (1 - pow(dot_prod(vector_u, vector_v), 2));
  rez_calc_b = (calc_b - calc_a * dot_prod(vector_u, vector_v)) /
               (1 - pow(dot_prod(vector_u, vector_v), 2));

  rez_calc_c = pow(rez_calc_a, 2) + pow(rez_calc_b, 2) +
               2 * rez_calc_a * rez_calc_b * dot_prod(vector_u, vector_v) +
               rez_calc_a * dot_prod(vector_u, vector_w) +
               rez_calc_b * dot_prod(vector_v, vector_w) - calc_c;

  REAL saknis1 = sqrt(pow(dot_prod(vector_w, vector_t), 2) - 4 * rez_calc_c);
  if (saknis1 > 0) {
    rez_calc_d_positive = (0.5) * (-dot_prod(vector_w, vector_t) +
                                   saknis1); // ar reikia minusines reiksmes
    ...
    rez_sphere_coords_positive = rez_calc_a * vector_u + rez_calc_b * vector_v +
                                 rez_calc_d_positive * vector_t;

    rez_sphere_coords_negative = rez_calc_a * vector_u + rez_calc_b * vector_v +
                                 rez_calc_d_negative * vector_t;

    rez_sphere_coords_positive.R = r;
    rez_sphere_coords_negative.R = r;
    rezults.push_back(rez_sphere_coords_positive);
    rezults.push_back(rez_sphere_coords_negative);
    // cout << rez_sphere_coords_negative.x << " " <<
    // rez_sphere_coords_negative.y << " " << rez_sphere_coords_negative.z <<
    // endl; cout << rez_sphere_coords_positive.x << " " <<
    // rez_sphere_coords_positive.y << " " << rez_sphere_coords_positive.z <<
    // endl;
