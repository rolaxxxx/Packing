...
std::vector<INT> SearchUnform::GetPotentialNeighbours(Point temp) {
  ...
  // cout << bmin.x<< " " << bmin.y << " " << bmin.z << " " << CELL_SIZE <<
  // endl;
  // cout << ((temp.x-bmin.x)/CELL_SIZE) << " " <<
  // (int)floor((temp.x-bmin.x)/CELL_SIZE) << endl; time.StartTimer();
...
std::vector<INT> SearchUnform::getNeighboursID(INT id) {
  ...
  // std::cout<<"Potencialus kaimynu skaicius
  // "<<potencialus_kaimynai.size()<<"\n";
  std::vector<INT> ids;
  // cout << potencialus_kaimynai.size() << endl;
  for (int i = 0; i < potencialus_kaimynai.size(); i++) {
    // std::cout<<"target id "<<p.ID<<"  potencialus kaimynas
    // "<<potencialus_kaimynai[i]<<"\n";
