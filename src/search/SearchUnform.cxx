...
bool SearchUnform::check_sphere_touch(PointType s1, PointType s2, PointType s3, PointType calculated_point){
    ...
    if(s1.R-calculated_point.R<=1e-9&&s2.R-calculated_point.R<=1e-9&&s3.R-calculated_point.R<=1e-9){ // paklaidos testavimas nes spinduliu skirtumas didesnis nei EPSILON
		return true;
	}
	else return false;
}
